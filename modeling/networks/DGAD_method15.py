...
class BN_layer(nn.Module):
    ...
    def _make_layer(self, block: Type[Union[BasicBlock, Bottleneck]], planes: int, blocks: int,
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            downsample = nn.Sequential(
                conv1x1(self.inplanes * 1, planes * block.expansion, stride),
                norm_layer(planes * block.expansion),
            )
...
def _BN_layer(
    ...
    if pretrained:
        state_dict = load_state_dict_from_url(model_urls[arch],
                                              progress=progress)
        for k,v in list(state_dict.items()):
           if not ('layer4.0' in k or 'layer2.0' in k or 'layer3.0' in k):
               state_dict.pop(k)
...
def resnet18(pretrained: bool = False, progress: bool = True, **kwargs: Any) -> ResNet:
    ...
    return _resnet('resnet18', BasicBlock, [2, 2, 2, 2], pretrained, progress,
                   **kwargs), _BN_layer("resnet18", AttnBottleneck, 2, 
...
def resnet34(pretrained: bool = False, progress: bool = True, **kwargs: Any) -> ResNet:
    ...
    return _resnet('resnet34', BasicBlock, [3, 4, 6, 3], pretrained, progress,
                   **kwargs), _BN_layer("resnet34", AttnBottleneck, 3, 
                                        pretrained, progress, **kwargs)
...
def resnet50(pretrained: bool = False, progress: bool = True, **kwargs: Any) -> ResNet:
    ...
    return _resnet('resnet50', Bottleneck, [3, 4, 6, 3], pretrained, progress,
                   **kwargs), _BN_layer("wide_resnet50_2", AttnBottleneck, 3, 
                                                             pretrained, progress, **kwargs)
