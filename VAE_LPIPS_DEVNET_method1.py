...
from modeling.VAE_LPIPS_DEVNET_method1 import SemiADNet
...
class Trainer(object):
    ...
    def train(self, epoch):
        ...
        self.scheduler.step()
        self.domain_key = "val"
        val_loss_list, val_auroc, val_auprc, total_pred, total_target, file_name_list = self.eval("val", self.val_loader)
    ...
    def test(self):
        ...
        for key in trainer.test_loader:
            self.domain_key = key
            print(key)
            test_loss_list, test_auroc, test_auprc, total_pred, total_target, file_name_list = self.eval(key, trainer.test_loader[key])
            test_metric[key] = {
    ...
    def eval(self, domain, dataloader):
        self.model.eval()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--backbone', type=str, default='VAE_LPIPS_DEVNET_1', help="the backbone network")
