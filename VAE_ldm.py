...
from modeling.VAE_ldm import SemiADNet
...
class Trainer(object):
    ...
    def train(self, epoch):
        ...
        for i, sample in enumerate(self.train_loader):
            _, image, augimg, target, domain_label, _ = sample

            if self.args.cuda:
                image, target, domain_label = image.cuda(), target.cuda(), domain_label.cuda()
            
            with torch.no_grad():
                reconstructions = self.model(image, domain_label, target)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--backbone', type=str, default='VAE_ldm', help="the backbone network")
    ...
    parser.add_argument("--method", type=str, default="VAE_ldm")
